...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        # TODO-BLOC-DEBUT
        Ix= scipy.ndimage.sobel(srcImage, axis=0, mode='reflect')
        Iy= scipy.ndimage.sobel(srcImage, axis=1, mode='reflect')

        Ix2 = ndimage.gaussian_filter(Ix**2, sigma=0.5, radius=2, mode='reflect')
        Iy2 = ndimage.gaussian_filter(Iy**2, sigma=0.5, radius=2, mode='reflect')
        Ixy = ndimage.gaussian_filter(Ix * Iy, sigma=0.5, radius=2, mode='reflect')
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        localMaxima = scipy.ndimage.maximum_filter(harrisImage, size=7)
        destImage = (harrisImage == localMaxima)

        destImage = destImage & (harrisImage > 0.001)
        # TODO-BLOC-FIN
    ...
    def detectKeypoints(self, image):
        ...
        for i in range(np.size(row)):
          ...
          # TODO-BLOC-DEBUT
          features = []
          #check the dummy example of the professor(above)
          for i in range(len(row)):
            f = cv2.KeyPoint()
            f.pt = (float(col[i]), float(row[i]))
            f.size = 10
            f.angle = orientationImage[row[i], col[i]]
            f.response = harrisImage[row[i], col[i]]
          # TODO-BLOC-FIN
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            # TODO-BLOC-DEBUT            
            patch = np.zeros((5, 5))
            #compute patch ela voisinnage de point clé
            for dy in range(-2, 3):  
                for dx in range(-2, 3):
                    nx, ny = x + dx, y + dy
                    if 0 <= nx < grayImage.shape[1] and 0 <= ny < grayImage.shape[0]:
                        patch[dy + 2, dx + 2] = grayImage[ny, nx]
                    else:
                        patch[dy + 2, dx + 2] = 0  
            #flatten to make it 1D array (5,5 becomes 25)
...
class MOPSFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            # TODO-BLOC-DEBUT
            x, y, angle = f.pt[0], f.pt[1], f.angle

            # T1 for the 40*40  translation
            T1 = np.array([[1, 0, -x], [0, 1, -y]])

            # R for rotation aka alignement 
           ...
           # S for scaling to 8*8
            scale = 8.0 / 40.0
            S = np.array([[scale, 0, 0], [0, scale, 0]])

            # T2 for second translation to the corner of the patch
            T2 = np.array([[1, 0, windowSize / 2], [0, 1, windowSize / 2]])
            # fixing dimensions pour la multiplication
            T1 = np.vstack([T1, [0, 0, 1]])  
            R = np.vstack([R, [0, 0, 1]])
            S = np.vstack([S, [0, 0, 1]])
            T2 = np.vstack([T2, [0, 0, 1]])

            #multiplication + fixing the dimensions back to 2,3
            transMx = (T2 @ S @ R @ T1)[:2, :]
            ...
            normalized = (destImage - mean) / np.sqrt(var)
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        # Calcule de distance euclédienne
        distance = scipy.spatial.distance.cdist(desc1, desc2, metric='sqeuclidean')

        # Looping through desc of img1
        for id1 in range(desc1.shape[0]):
            # Finding the index of the closest descriptor in desc2 
            id2 = np.argmin(distance[id1])  
            #verification de l'autre coté
            id11 = np.argmin(distance[:, id2])
            if id11 != id1:
                continue 

            bestDis = distance[id1, id2]  # storing the best dis
            bestDis = np.float32(bestDis)
            # storing it
            match = cv2.DMatch(id1, id2, bestDis)
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i in range(distance.shape[0]): 
            # sorting
            sortedIndx = np.argsort(distance[i])  
            ...
            # best et second best
            bestIndx = sortedIndx[0]
            secondBestIndx = sortedIndx[1]
            
            bestDis= distance[i, bestIndx]
            secondBestDis = distance[i, secondBestIndx]

            reversedIndx = np.argmin(distance[:, bestIndx])
            if reversedIndx != i:
                continue 
            reversedIndx2 = np.argmin(distance[:, secondBestIndx])
            if reversedIndx2 != i:
                continue 


            # check ur notes (day 02/03/2025)
            
            if secondBestDis >0 :
                ratio = bestDis / secondBestDis
                if ratio < 0.7:  # threshold
                    match = cv2.DMatch(i, bestIndx, np.float32(ratio))
                    matches.append(match)
